...
def classify_ensemble_topn(features, true_classes, classes_train, train, topn):
    accuracy = 0
    if train:
        logistic_regression = LogisticRegression(solver='liblinear', penalty='l2', multi_class='ovr', max_iter=50000, C=20.0)
        logistic_regression.fit(features, true_classes)
        pickle.dump(logistic_regression, open('ensemble_liblinear.pickle', 'wb'))
    else:
        logistic_regression = pickle.load(open('ensemble_liblinear.pickle', 'rb'))
...
print(np.mean(accuracies_top3))
print(np.mean(accuracies_top5))
